...
class InterfaceTest:
    def interface_test(self,num,api_purpose,api_host,request_url,request_data,check_point,request_method,request_data_type,i,table,log):
        full_request_url=api_host+request_url
        #注意从表里取出来的都是str类型，要用eval转成字典
        request_data=eval(request_data)
        
        http=HttpRequestResponse()
        if request_method=='POST'and request_data_type=='FORM':
            response,status_code,time=http.post_form(full_request_url,request_data_type)
        elif request_method=='GET':
            response,status_code,time=http.get(full_request_url,request_data_type)
        elif request_method=='POST'and request_data_type=='JSON':
            response,status_code,time=http.post_json(full_request_url,request_data_type)
        else:
